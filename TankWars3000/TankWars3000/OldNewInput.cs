...
namespace TankWars3000
{
    class OldNewInput
    {
        ...
        {
            get { return new Rectangle(newMouse.X, newMouse.Y, 1, 1); }
        }
        ...
        public bool SingleKey(Keys key)
        {
            if (newKey.IsKeyDown(key) && oldKey.IsKeyUp(key))
                return true;
            else
                return false;
        }
        ...
        public bool TryConvertKeyboardInput(out char key)
        {
            ...
            bool shift = newKey.IsKeyDown(Keys.LeftShift) || newKey.IsKeyDown(Keys.RightShift);

            if (keys.Length > 0 && !oldKey.IsKeyDown(keys[0]))
            {
                switch (keys[0])
                {
                    //Alphabet keys
